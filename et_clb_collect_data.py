...
(
    ...
    red_point_diameter,
    d
) = efp.get_calibration_win1()
dx, dy = d
calibration_win_width, calibration_win_height = calibration_win_size
...
t1 = time.time()
smp_now_y = red_point_diameter // 2
for yp in range(red_point_diameter // 2, calibration_win_height - red_point_diameter // 2):
    if yp == smp_now_y:
        cap = efp.get_camera()
        smp_now_x = red_point_diameter // 2
        calibration_img = (np.ones((calibration_win_size[1], calibration_win_size[0], 3)) * 255)
        cv2.circle(calibration_img, (smp_now_x, yp), red_point_diameter, (0, 0, 255), cv2.FILLED)
        cv2.putText(calibration_img, f"{p}",
                    (int(smp_now_x - red_point_diameter // 1.5), int(yp + red_point_diameter // 2.7)),
        ...
        if button == 27:
            ...
            for xp in range(red_point_diameter // 2, calibration_win_width - red_point_diameter // 2):
                if xp == smp_now_x:
                    calibration_img = (np.ones((calibration_win_size[1], calibration_win_size[0], 3)) * 255)
                    cv2.circle(calibration_img, (xp, yp), red_point_diameter, (0, 0, 255), cv2.FILLED)
                    cv2.putText(calibration_img, f"{p}",
                                (int(xp - red_point_diameter // 1.5), int(yp + red_point_diameter // 2.7)),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 3)
                    cv2.namedWindow("Calibration")
                    cv2.moveWindow("Calibration", tp.CALIBRATION_WIN_X, tp.CALIBRATION_WIN_Y)
                    cv2.imshow("Calibration", calibration_img)
