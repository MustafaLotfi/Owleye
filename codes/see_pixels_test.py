...
subject_dir = "../subjects/" + f"{tp.NUMBER}/" + "sampling_test data-pixels/"
...
for (y_ibo0, t, px_hat, px) in zip(y_hat_smp_ibo, t_vec, y_hat, y):
    print(px_hat)
    show_img = (np.ones((show_win_size[1], show_win_size[0], 3)) * 255).astype(np.uint8)
    cv2.circle(show_img, px, pnt_d, (0, 0, 255), cv2.FILLED)
    if True:  # y_ibo0 == 0:
